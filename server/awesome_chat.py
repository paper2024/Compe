...
def get_description(url):
    ...
    if response.status_code == 200:
        ...
        prose_elements = soup.select('.prose p')
        for i, p_element in enumerate(prose_elements[:3]):
            all_text += p_element.get_text()
        return all_text
    else:
        return "Nothing"
...
def get_model_info(model_id):
    ...
    if response.status_code == 200:
        ...
        formatted_info = {
            "downloads": model_info.get("downloads", "nothing"),
            "id": model_info.get("id", "nothing"),
            "likes": model_info.get("likes", "nothing"),
            "pipeline_tag": model_info.get("pipeline_tag", "nothing"),
            "task": model_info.get("pipeline_tag", "nothing"),
            "meta": {
                "language": model_info["cardData"].get("language", "nothing"),
                "tags": model_info.get("tags", "nothing"),
                "widget": model_info.get("widgetData", "nothing")
            },
            "description": get_description(f'https://huggingface.co/{model_id}')
        }
        return formatted_info
    else:
        print(f"Error: Unable to fetch information for model {model_id}")
        return None


# Get the top models by order of ['downloads', 'likes', 'trending']
def get_sort(task_type):
    models_info = []
    # order_by in ['downloads', 'likes', 'trending']
    for order_by in ['downloads', 'likes', 'trending']:
        url = "https://huggingface.co/models?pipeline_tag={}&sort={}".format(task_type, order_by)
        response = requests.get(url)
        soup = BeautifulSoup(response.text, 'html.parser')
        target_element = soup.select_one('article.overview-card-wrapper:nth-child({})'.format(1))
        nextsoup = BeautifulSoup(target_element.prettify(), 'html.parser')
        repo_article = nextsoup.find('article', class_='overview-card-wrapper')
        repo_name = repo_article.find('h4', class_='text-md').text.strip()
        models_info.append(get_model_info(repo_name))
    return models_info
    
def extract_bracket_content_with_brackets(text):
...
def calculate_similarity_matrix(sentences, model_name='paraphrase-distilroberta-base-v1'):
    model = SentenceTransformer(model_name)
    sentence_embeddings = model.encode(sentences, convert_to_tensor=True)
    cosine_sim_matrix = util.pytorch_cos_sim(sentence_embeddings, sentence_embeddings)
    return cosine_sim_matrix

def let_llm_choose(result_A, result_B, result_C, if_text_generate, LLM_model="gpt-3.5-turbo"):
    client = OpenAI(
        api_key=API_KEY
    )
    if if_text_generate:
        sim_m = calculate_similarity_matrix([result_A, result_B, result_C])
        prompt_text = ('The following are three results given by three pre-trained models facing the same subtask: '
                       'result1:{}; result2:{}; result3:{}. '
                       'The similarity matrix of these three results is {}. '
                       'Please provide me with the most possible correct answer '
                       'based on the similarity matrix '
                       'and tell me through a dictionary like {{"the most possible correct answer":result1, result2 or result3}} strictly'.format(result_A, result_B, result_C, sim_m))
    else:
        prompt_text = ('The following are three results given by three pre-trained models facing the same subtask: '
                       'result1:{}; result2:{}; result3:{}. '
                       'Please provide me with the most possible correct answer '
                       'and tell me through a dictionary like {{"the most possible correct answer":result1, result2 or result3}} strictly'.format(result_A, result_B, result_C))
...
def run_task(input, command, results, api_key, api_type, api_endpoint):
    ...
    if task.endswith("-text-to-image") or task.endswith("-control"):
        ...
        models_results = []
        for i in range(3):
            best_model_id = candidates[i]['id']
            models_results.append(model_inference(best_model_id, args, 'huggingface', command['task']))
        if 'generated text' in list(models_results[0].keys())[0]:
            candidates_choosed = extract_bracket_content_with_brackets(let_llm_choose(list(models_results[0].values())[0], list(models_results[0].values())[0], list(models_results[0].values())[0], True))
        else:
            candidates_choosed = extract_bracket_content_with_brackets(let_llm_choose(models_results[0], models_results[1], models_results[2], False))
        best_model_id = candidates[eval(candidates_choosed)-1]['id']
        hosted_on = "local" if best_model_id in all_avaliable_models["local"] else "huggingface"
        reason = "LLM think this is the best."
        choose = {"id": best_model_id, "reason": reason}
        logger.debug(f"chosen model: {choose}")
    inference_result = model_inference(best_model_id, args, 'huggingface', command['task'])
...
def cli():
    messages = []
    print("Welcome to CompeGPT! A collaborative system that consists of an LLM as the controller and numerous expert models as collaborative executors. CompGPT can plan tasks, schedule Hugging Face models, generate friendly responses based on your requests, and help you with many things. Please enter your request (`exit` to exit).")
    while True:
        message = input("[ User ]: ")
        if message == "exit":
            break
